"""Test Glm Embedding connection.

This script validates the Glm Embedding configuration and API connectivity.
...
from src.libs.embedding.glm_embedding import GlmEmbedding, GlmEmbeddingError


def test_Glm_embedding_connection():
    """Test Glm Embedding connection with minimal setup."""
    print("=" * 50)
    print("Testing Glm Embedding Connection")
    print("=" * 50)
    ...
    try:
        ...
        # Try to get Glm embedding config from additional_embeddings
        glm_config = None
        if hasattr(settings, 'additional_embeddings') and settings.additional_embeddings and "glm" in settings.additional_embeddings:
            glm_config = settings.additional_embeddings["glm"]
            print(f"  Found glm in additional_embeddings")
        else:
            # Fallback to main embedding config
            glm_config = settings.embedding
            print(f"  Using main embedding configuration")

        print(f"  Provider: {glm_config.provider}")
        print(f"  Model: {glm_config.model}")
        print(f"  Dimensions: {getattr(glm_config, 'dimensions', 'Not specified')}")
    except Exception as e:
        print(f"✗ ERROR loading settings: {e}")
        return False

    # Create Glm Embedding instance
    try:
        embedding = GlmEmbedding(settings=settings, embedding_config=glm_config)
        print(f"✓ GlmEmbedding instance created")
    ...
    except GlmEmbeddingError as e:
        print(f"✗ Glm Embedding API error: {e}")
        return False
    except Exception as e:
    ...
    print(f"\n{'=' * 50}")
    print("✓ Glm Embedding Connection Test: SUCCESS")
    print(f"{'=' * 50}")
    print("\nSummary:")
...
if __name__ == "__main__":
    success = test_Glm_embedding_connection()
