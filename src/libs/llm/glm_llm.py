"""Glm LLM implementation.

This module provides the Glm LLM implementation that works with
Glm's API through DashScope's compatible-mode endpoint.
"""
...
class GlmLLM(BaseLLM):
    ...
    This class implements the BaseLLM interface for Glm's chat API.
    Glm uses a DashScope-compatible API format.
    ...
    def __init__(
        ...
        Args:
            ...
            api_key: Optional API key override (falls back to env var Glm_API_KEY).
        ...
        elif hasattr(settings, 'additional_llms') and settings.additional_llms and 'Glm' in settings.additional_llms:
            # 自动从 additional_llms 中获取 Glm 配置
            llm_settings = settings.additional_llms['Glm']
        else:
            # 回退到主 llm 配置
    ...
    def chat(
        ...
        try:
            ...
            raise GlmLLMError(
                f"[GLM] Unexpected response format: missing key {e}"
            ) from e
        except Exception as e:
            if isinstance(e, GlmLLMError):
                raise
            raise GlmLLMError(
                f"[GLM] API call failed: {type(e).__name__}: {e}"
            ) from e
    ...
    def _call_api(
        ...
        Raises:
            GlmLLMError: If the API call fails.
        """
        import httpx
