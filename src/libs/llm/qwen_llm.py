...
class QwenLLM(BaseLLM):
    ...
    def chat_stream(
        ...
        """Generate a streaming chat completion using Qwen API.
        ...
        try:
            ...
            if isinstance(e, QwenLLMError):
                raise
            raise QwenLLMError(
                f"[Qwen] Stream API call failed: {type(e).__name__}: {e}"
            ) from e
    ...
    def _call_api_stream(
        ...
        """Make a streaming API call to Qwen.
        ...
        with httpx.Client(timeout=120.0) as client:
            with client.stream("POST", url, json=payload, headers=headers) as response:
                if response.status_code != 200:
                    ...
                    raise QwenLLMError(
                        f"[Qwen] Stream API error (HTTP {response.status_code}): {error_detail}"
                    )
